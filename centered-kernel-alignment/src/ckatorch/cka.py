...
class CKA:
    ...
    def _hook(self, model: str, module_name: str, module: nn.Module, inp, out) -> None:
        # 만약 out이 GeometricTensor라면 내부 tensor를 사용합니다.
    ...
    # def _hook(self, model: str, module_name: str, module: nn.Module, inp, out) -> None:
    #     # 만약 출력 텐서가 2차원보다 크면 flatten 처리 ([B, ...] -> [B, -1])
    #     if out.dim() > 2:
    #         out = out.view(out.size(0), -1)
    #     if model == "first":
    #         self.first_features[module_name] = out.detach()
    #     else:
    #         self.second_features[module_name] = out.detach() #cnn까지 잘 동작
    ...
    #         self.first_features[module_name] = out.detach()
    #     else:
    #         self.second_features[module_name] = out.detach() #linear 동작

    # def _hook(self, model: str, module_name: str, module: nn.Module, inp: torch.Tensor, out: torch.Tensor) -> None:
    #     del module, inp  # 사용하지 않는 인자 삭제
    #     # out이 2D라면 3D로 변환 (예: 첫 번째 차원에 추가)
    #     if out.dim() == 2:
    #         out = out.unsqueeze(1)
    #     if model == "first":
    #         self.first_features[module_name] = out.detach()
    #     else:
    #         self.second_features[module_name] = out.detach() #원형
