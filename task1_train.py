...
# --- 평가 함수 ---
def test(model, device, dataloader, criterion, task_name="Task1"):
    model.eval()
    test_loss = 0
    correct = 0
    total = 0
...
# --- 메인 함수: Task1 학습, 모델 및 로그 저장 ---
def main(task1_samples_per_class=1000, batch_size=64, epochs=5):
    ...
    # 데이터 증강 없이 ToTensor만 적용 (Task1)
    transform = transforms.ToTensor()
    train_dataset = MNIST(root='./data', train=True, download=True, transform=transform)
    test_dataset = MNIST(root='./data', train=False, download=True, transform=transform)
    
    # 각 클래스별 동일한 샘플 수 선택
    train_subset = get_subset_by_class(train_dataset, task1_samples_per_class)
    ...
    for epoch in range(1, epochs + 1):
        avg_train_loss = train_task(model, device, train_loader, optimizer, criterion, epoch, task_name="Task1")
        test_loss, test_acc = test(model, device, test_loader, criterion, task_name="Task1")
    ...
    print("Model saved as model_task1.pth")
    
    # 학습 로그 저장 (JSON 파일)
    with open("training_history_task1.json", "w") as f:
        json.dump(training_history, f, indent=4)
    print("Training history saved as training_history_task1.json")

if __name__ == '__main__':
    main(task1_samples_per_class=1000, batch_size=64, epochs=5)
